...
df %>%
  ...
  mutate(ALG.num = sqrt(ALG.num)) %>%
  summarise(cor_pearson = cor(ALG.num, MD.num, use = "complete.obs", method = "pearson"))

# Spearman
df %>%
  select(ends_with(".num")) %>%
  mutate(ALG.num = sqrt(ALG.num)) %>%
  summarise(cor_spearman = cor(ALG.num, MD.num, use = "complete.obs", method = "spearman"))
```
