...
# Pearson
df %>%
  mutate(ALG.num_sqrt = sqrt(ALG.num)) %>%
  summarise(cor_pearson = cor(ALG.num_sqrt, MD.num, use = "complete.obs", method = "pearson"))

# Spearman
df %>%
  mutate(ALG.num_sqrt = sqrt(ALG.num)) %>%
  summarise(cor_spearman = cor(ALG.num_sqrt, MD.num, use = "complete.obs", method = "spearman"))
