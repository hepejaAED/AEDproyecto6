...
df %>% 
  pivot_longer(cols=ends_with(".num"),names_to = "asignatura",values_to = "valor") %>% 
  ggplot(aes(valor))+
  geom_histogram(aes(y=stat(density)))+ geom_density() 
```
...
library(car)
valores <- df %>% pivot_longer(cols = ends_with('.num'), names_to = 'asignatura', values_to = 'valor')%>%filter(asignatura == "ANM.num")
...
alg<-table(df$ALG.cat)
alg
ALG_ANM<-table(df$ALG.cat,df$ANM.cat)
ALG_ANM
mosaicplot(ALG_ANM)
```
...
df %>% select(ends_with(".num")) %>% cor(use="complete.obs",method="pearson")
df %>% select(ends_with(".num")) %>% cor(use="complete.obs",method="spearman") # Por lo general spearman es más robusto pero nos interesa siempre calcularlo todo
```
...
# Pearson
df %>%
  mutate(ALG.num_sqrt = sqrt(ALG.num)) %>% select(c(ALG.num_sqrt, MD.num))%>% cor( use = "complete.obs", method = "pearson")

# Spearman
df %>%
  mutate(ALG.num_sqrt = sqrt(ALG.num)) %>% select(c(ALG.num_sqrt, MD.num)) %>% cor( use = "complete.obs", method = "spearman")
...
df %>%  select(ends_with(".num")) %>% ggpairs()
```
...
df %>% select(c(ALG.cat,ANM.cat)) %>% table() %>% chisq.test(correct = F)
```


```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
df %>% select(c(ALG.cat,Sex)) %>% table() %>% chisq.test(correct = F)
```
